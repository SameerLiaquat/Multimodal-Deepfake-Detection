...
class DINOEncoder(nn.Module):
    def __init__(self, model_name="facebook/dinov2-with-registers-base"):
        ...
        # DINOv3 models don't have preprocessor_config on HuggingFace yet
        # Fall back to DINOv2 processor which is compatible
        if "dinov3" in model_name.lower():
            processor_name = "facebook/dinov2-with-registers-large"
        else:
            processor_name = model_name

        self._preprocess = AutoImageProcessor.from_pretrained(processor_name)
        import torch
        with torch.device('cpu'):
            self.backbone = AutoModel.from_pretrained(model_name, device_map=None, low_cpu_mem_usage=False)
