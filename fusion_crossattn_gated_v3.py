...
def crop_face_pil(pil_img, margin=1.3):
    """Same Haar-cascade crop already validated for Face X-Ray, reused here.
    GenD's own native classifier confirmed to need this too: 78.5% frame-AUC
    uncropped vs 96.1% cropped on a 120-video check."""
...
def extract_gend_features(model, image_path):
    try:
        ...
        img = crop_face_pil(img)  # FIX: GenD needs the same face crop Face X-Ray needed
...
def main():
    ...
    save_path = os.path.join(SAVE_DIR, 'fusion_crossattn_gated_v3_best.pth')
    ...
    print(f'\nComparison:')
    print(f'  GenD alone, cropped, frame-level : 96.140%  (confirmed reference)')
    print(f'  GenD alone, cropped, video-level : 98.556%  (confirmed reference)')
    print(f'  GenD alone, UNCROPPED (old runs) : ~78-92%  (unreliable, do not use)')
    print(f'  Face X-Ray alone, cropped        : ~87%     (600-sample check)')
    print(f'  Arch 2b Gated v3 (both cropped)  : {best_val_auc:.3f}%')
    print(f'{"="*60}')
